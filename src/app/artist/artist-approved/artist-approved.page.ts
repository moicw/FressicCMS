...
@Component({
  selector: 'app-artist-approved',
  templateUrl: './artist-approved.page.html',
  styleUrls: ['./artist-approved.page.scss'],
})
export class ArtistApprovedPage implements OnInit {
  ...
  selected_ethnic:any;
  ngOnInit() {
    this.api.load_approvedartistlist().then((result) => {
  ...
  save_artist()
  {
    ...
    let author_username = this.selected_artist.data.username;
      
    for (let i = 1; i <= author_username.length; i++) { 
      name_keyword.push(author_username.toLowerCase().substring(0, i)); 
    } 
  ...
  selected_track_to_play(num)
  {
    this.selected_track_info = this.track_info[num];
  ...
  fileChanged(event) {
    ...
    reader.onload = () => {
      ...
      this.selected_artist.image.artist_cover_image = reader.result as string;
